...
private:
    ...
    // Point Cloud
    pcl::PointCloud<PointType>::Ptr laserCloudIn;        // projected full hesai cloud
    ...
    std::vector<std::vector<bool>> initFlag;
    std::vector<std::vector<bool>> obstFlag;
    std::vector<std::vector<float>> minHeight;
...
public:
                             ...
                             tf_listener(tf_buffer)
    {
        ...
        pubCloud = this->create_publisher<sensor_msgs::msg::PointCloud2>(
            "/filtered_pointcloud", rclcpp::QoS(5));
        pubCloudVisualHiRes = this->create_publisher<sensor_msgs::msg::PointCloud2>(
            "/filtered_pointcloud_visual_high_res", rclcpp::QoS(5));
        pubCloudVisualLowRes = this->create_publisher<sensor_msgs::msg::PointCloud2>(
            "/filtered_pointcloud_visual_low_res", rclcpp::QoS(5));
        pubLaserScan = this->create_publisher<sensor_msgs::msg::LaserScan>(
            "/pointcloud_2_laserscan", rclcpp::QoS(5));
        allocateMemory();
        pointcloud2laserscanInitialization();

        RCLCPP_INFO(this->get_logger(), "Traversability Filter Started.");
    }
    ...
    void allocateMemory()
    {
        ...
        laserCloudMatrix.resize(N_SCAN, std::vector<PointType>(Horizon_SCAN));

        initFlag.resize(filterHeightMapArrayLength, std::vector<bool>(filterHeightMapArrayLength, false));
        obstFlag.resize(filterHeightMapArrayLength, std::vector<bool>(filterHeightMapArrayLength, false));
        minHeight.resize(filterHeightMapArrayLength, std::vector<float>(filterHeightMapArrayLength, 0.0f));
        maxHeight.resize(filterHeightMapArrayLength, std::vector<float>(filterHeightMapArrayLength, 0.0f));
    ...
    bool transformCloud()
    {
        ...
        pcl::transformPointCloud(*laserCloudIn, laserCloudTemp, eigen_transform);
        ...
        for (int i = 0; i < scanNumCurbFilter; ++i)
        {
            for (int j = 0; j < Horizon_SCAN; ++j)
            {
                ...
                for (int m = -rangeCompareNeighborNum; m <= rangeCompareNeighborNum; ++m)
                {
                    ...
                    if (laserCloudMatrix[i][j].z - laserCloudMatrix[i][k].z > 0.1 && pointDistance(laserCloudMatrix[i][j], laserCloudMatrix[i][k]) <= 1.0)
                    {
                        obstacleMatrix.at<int>(i, j) = 1;
                        break;
                    }
    ...
    void predictCloudBGK()
    {
        ...
        int kernelGridLength = static_cast<int>(predictionKernalSize / mapResolution);
        ...
        for (int i = 0; i < filterHeightMapArrayLength; ++i)
        {
            for (int j = 0; j < filterHeightMapArrayLength; ++j)
            {
                ...
                std::vector<float> xTrainVec;
                std::vector<float> yTrainVecElev;
                std::vector<float> yTrainVecOccu;
                ...
                Eigen::MatrixXf xTrain = Eigen::Map<const Eigen::Matrix<float, -1, -1, Eigen::RowMajor>>(
                ...
                std::vector<float> xTestVec = { testPoint.x, testPoint.y };
                
                Eigen::MatrixXf xTest = Eigen::Map<const Eigen::Matrix<float, -1, -1, Eigen::RowMajor>>(
                ...
                p.y = xTestVec[1];
                p.z = elevation;
                p.intensity = (occupancy > 0.5f) ? 100 : 0;
    ...
    void updateLaserScan()
    {
        ...
        pcl::PointCloud<PointType> laserCloudTemp;
        Eigen::Affine3d eigen_transform = tf2::transformToEigen(transform);
        pcl::transformPointCloud(*laserCloudObstacles, laserCloudTemp, eigen_transform);
        ...
        for (int i = 0; i < cloudSize; ++i)
        {
            ...
            if (index >= 0 && index < static_cast<int>(laserScan.ranges.size()))
...
int main(int argc, char **argv)
{
    ...
    RCLCPP_INFO(
        node->get_logger(),
        "\033[1;32m---->\033[0m Traversability Filter Started.");
    rclcpp::spin(node);
